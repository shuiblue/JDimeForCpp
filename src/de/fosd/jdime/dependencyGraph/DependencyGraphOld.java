 ...
 */
public class DependencyGraphOld {
    ...
    public static HashSet<Edge> createDependencyGraph(String testDir) {
        ...
        for (int i = 0; i < declarationNodes.size(); i++) {
            for (int j = 0; j < dependenceNodes.size(); j++) {
                ...
                if (decl.getName().equals(depen.getName())) {
                    ...
                    edges.add(new Edge(edgeLable,depenNode,declNode));
                }
            }
        }
    ...
    /**
     * add  a node (line of code + file name) to nodeList
     *
     * @param nodeLabel node name
     * @param num       node id
     */
     ...
     * @param fileName    one of the file
     */
     ...
     *
     * @param xmlFilePath xmlfile path
     * @param query       xpath query
     * @param decl_tag    tag from srcml
