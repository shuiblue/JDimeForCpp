...
//the SD start is delayed, because otherwise the serial cannot answer fast enough to make contact with the host software.
    ...
    lastnr=0;
//power to SD reader
#if defined (upstream) && (SDPOWER > -1 && defined (SDSUPPORT) ) || defined (mattsch) && (SDPOWER > -1 && defined (SDSUPPORT) ) || defined (mattschUpstream) && (SDPOWER > -1 && defined (SDSUPPORT) )
    SET_OUTPUT(SDPOWER);
    WRITE(SDPOWER,HIGH);
#endif
//SDPOWER
...
#endif
#if defined (upstream) && defined (SDSUPPORT) || defined (mattschUpstream) && defined (SDSUPPORT)
