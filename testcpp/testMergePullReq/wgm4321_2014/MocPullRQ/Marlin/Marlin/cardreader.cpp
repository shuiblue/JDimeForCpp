...
//the SD start is delayed, because otherwise the serial cannot answer fast enough to make contact with the host software.
    lastnr=0;
//power to SD reader
#if defined (upstreamBeforeMerge) && (SDPOWER > -1 && defined (SDSUPPORT) ) || defined (forkBeforeMerge) && (SDPOWER > -1 && defined (SDSUPPORT) )
    SET_OUTPUT(SDPOWER);
    WRITE(SDPOWER,HIGH);
#endif
//SDPOWER
...
() {
#if defined (upstreamBeforeMerge) && defined (SDSUPPORT)
    {
        cardOK = false;
        if(root.isOpen())
            root.close();
