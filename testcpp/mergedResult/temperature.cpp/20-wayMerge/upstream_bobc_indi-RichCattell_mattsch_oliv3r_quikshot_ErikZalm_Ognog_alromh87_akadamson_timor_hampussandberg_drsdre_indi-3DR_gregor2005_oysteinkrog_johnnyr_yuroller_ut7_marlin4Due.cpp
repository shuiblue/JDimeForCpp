    ...
    */
    ...
    //===========================================================================
#endif
#if defined (marlin4Due) && defined (K1)
    // Defined in Configuration.h in the PID settings
    #define K2 (1.0-K1)
#endif
#if defined (marlin4Due) && (ENABLED(PIDTEMPBED) || ENABLED(PIDTEMP))
...
float current_temperature_bed = 0.0;
#endif
#if defined (marlin4Due) && ENABLED(TEMP_SENSOR_1_AS_REDUNDANT)
    ...
    //PIDTEMPBED
#endif
#if defined (marlin4Due) && ENABLED(FAN_SOFT_PWM)
    unsigned char fanSpeedSoftPwm;
#endif
#if defined (marlin4Due)
    unsigned char soft_pwm_bed;
#endif
#if defined (marlin4Due) && ENABLED(BABYSTEPPING)
...
#endif
#if defined (marlin4Due) && (ENABLED(THERMAL_PROTECTION_HOTENDS) || ENABLED(THERMAL_PROTECTION_BED))
enum TRState { TRReset, TRInactive, TRFirstHeating, TRStable, TRRunaway };
void thermal_runaway_protection(TRState *state, millis_t *timer, float temperature, float target_temperature, int heater_id, int period_seconds, int hysteresis_degc);
#endif
#if defined (marlin4Due) && ((ENABLED(THERMAL_PROTECTION_HOTENDS) || ENABLED(THERMAL_PROTECTION_BED) && )ENABLED(THERMAL_PROTECTION_HOTENDS) )
static TRState thermal_runaway_state_machine[4] = { TRReset, TRReset, TRReset, TRReset };
static millis_t thermal_runaway_timer[4];
// = {0,0,0,0};
#endif
#if defined (marlin4Due) && ((ENABLED(THERMAL_PROTECTION_HOTENDS) || ENABLED(THERMAL_PROTECTION_BED) && )(ENABLED(THERMAL_PROTECTION_BED) && TEMP_SENSOR_BED != 0 &)
...
#endif
#if defined (marlin4Due) && HAS_AUTO_FAN
    static millis_t next_auto_fan_check_ms;
#endif
#if defined (marlin4Due) && (ENABLED(PID_PARAMS_PER_EXTRUDER) && ENABLED(PIDTEMP) )
...
#endif
#if defined (marlin4Due) && (ENABLED(PID_PARAMS_PER_EXTRUDER) && ENABLED(PIDTEMP) && ENABLED(PID_ADD_EXTRUSION_RATE) )
...
#endif
#if defined (marlin4Due) && defined (BED_MINTEMP)
    static int bed_minttemp_raw = HEATER_BED_RAW_LO_TEMP;
#endif
#if defined (marlin4Due) && defined (BED_MAXTEMP)
...
#endif
#if defined (marlin4Due) && ENABLED(THERMAL_PROTECTION_HOTENDS)
int watch_target_temp[EXTRUDERS] = { 0 };
millis_t watch_heater_next_ms[EXTRUDERS] = { 0 };
#endif
#if defined (marlin4Due) && !defined (SOFT_PWM_SCALE)
...
#endif
#if defined (marlin4Due) && ENABLED(HEATER_0_USES_MAX6675)
            ...
            if (!heating && input < temp) {
                if (ms > t1 + 5000) {
                    ...
                    if (cycles > 0) {
                        ...
                        SERIAL_PROTOCOLPGM(MSG_BIAS);
                        SERIAL_PROTOCOL(bias);
                        SERIAL_PROTOCOLPGM(MSG_D);
                        SERIAL_PROTOCOL(d);
                        SERIAL_PROTOCOLPGM(MSG_T_MIN);
                        SERIAL_PROTOCOL(min);
                        SERIAL_PROTOCOLPGM(MSG_T_MAX);
                        SERIAL_PROTOCOLLN(max);
                        if (cycles > 2) {
                            Ku = (4.0 * d) / (3.14159265 * (max - min) / 2.0);
                            Tu = ((float)(t_low + t_high) / 1000.0);
                            SERIAL_PROTOCOLPGM(MSG_KU);
                            SERIAL_PROTOCOL(Ku);
                            SERIAL_PROTOCOLPGM(MSG_TU);
                            SERIAL_PROTOCOLLN(Tu);
                            Kp = 0.6 * Ku;
                            Ki = 2 * Kp / Tu;
                            Kd = Kp * Tu / 8;
                            SERIAL_PROTOCOLLNPGM(MSG_CLASSIC_PID);
                            SERIAL_PROTOCOLPGM(MSG_KP);
                            SERIAL_PROTOCOLLN(Kp);
                            SERIAL_PROTOCOLPGM(MSG_KI);
                            SERIAL_PROTOCOLLN(Ki);
                            SERIAL_PROTOCOLPGM(MSG_KD);
...
#define MAX_OVERSHOOT_PID_AUTOTUNE 20
        ...
        if (cycles > ncycles) {
            ...
            SERIAL_PROTOCOLPGM("#define  DEFAULT_");
            SERIAL_PROTOCOL(estring);
...
inline void _temp_error(int e, const char *serial_msg, const char *lcd_msg) {
    ...
    if (IsRunning()) {
        ...
        else SERIAL_ERRORLNPGM(MSG_HEATER_BED);
    }
