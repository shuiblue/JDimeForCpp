...
#if EXTRUDERS > 1
        soft_pwm_1 = soft_pwm[1];
        if(soft_pwm_1 > 0) WRITE_HEATER(HEATER_1_PIN,1);
        else WRITE_HEATER(HEATER_1_PIN,0);
#endif
#if EXTRUDERS > 2
        soft_pwm_2 = soft_pwm[2];
        if(soft_pwm_2 > 0) WRITE_HEATER(HEATER_2_PIN,1);
        else WRITE_HEATER(HEATER_2_PIN,0);
#endif
#if defined(HEATER_BED_PIN) && HEATER_BED_PIN > -1
        soft_pwm_b = soft_pwm_bed;
        if(soft_pwm_b > 0) WRITE_HEATER(HEATER_BED_PIN,1);
        else WRITE_HEATER(HEATER_BED_PIN,0);
#endif
#ifdef FAN_SOFT_PWM
        soft_pwm_fan = fanSpeedSoftPwm / 2;
        if(soft_pwm_fan > 0) WRITE(FAN_PIN,1);
