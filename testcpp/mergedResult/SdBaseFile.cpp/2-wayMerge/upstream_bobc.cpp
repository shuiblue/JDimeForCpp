...
void SdBaseFile::getpos
    ...
    (fpos_t* pos)
#endif
#if defined (bobc) && defined (SDSUPPORT)
    (FatPos_t* pos)
#endif
{
...
#if defined (bobc) && (defined (SDSUPPORT) && defined (__AVR__) )
// store chars in flash
#endif
#if defined (upstream) && defined (SDSUPPORT) || defined (bobc) && (defined (SDSUPPORT) && defined (__AVR__) )
            PGM_P p = PSTR("|<>^+=?/[];,*\"\\");
            uint8_t b;
            while
            ((b = pgm_read_byte(p++)))
...
#endif
#endif
#if defined (bobc) && (!defined (__AVR__) && defined (SDSUPPORT) )
// __AVR__
// store chars in RAM
            if (strchr("|<>^+=?/[];,*\"\\", c)) {
                goto fail;
...
// long filename entries are stored before the actual filename.
        if
        (DIR_IS_LONG_NAME(dir) && longFilename != NULL) {
            vfat_t *VFAT = (vfat_t*)dir;
//Sanity check the VFAT entry. The first cluster is always set to zero. And th esequence number should be higher then 0
            if
            (VFAT->firstClusterLow == 0 && (VFAT->sequenceNumber & 0x1F) > 0 && (VFAT->sequenceNumber & 0x1F) <= MAX_VFAT_ENTRIES) {
...
#endif
#if defined (upstream) && ((ALLOW_DEPRECATED_FUNCTIONS && !defined(DOXYGEN) && )defined (SDSUPPORT) ) || defined (bobc) && ((ALLOW_DEPRECATED_FUNCTIONS && !defined(DOXYGEN) && )defined (SDSUPPORT) )
